...
**混淆方向的耗时不在算法里**：生成混淆图时，算法只占整条链路的百分之几（1080p 小番茄
约 23 ms），大头是系统的图片解码与 PNG 编码。所以 v1.7.2 额外做了一件事：混淆结果（噪声图）
不再走系统编码器，改用 `FastPng`——行滤波恒为 0、整条 zlib 流按行分段并行压缩
（裸 deflate + `Z_FULL_FLUSH` 拼接，外面套 zlib 头与全量 Adler-32，这是并行 gzip 的标准做法），
输出仍是标准 PNG。真机 1080p 噪声实测 **53 ms**，体积比系统编码器大 1% 以内；
还原方向（照片输出）保留系统编码器，因为照片用滤波能把体积压下来。
